libsvm.for.parameter <- function(){
	...
	############################################################
	write.table("C G ACC AUC MCC SEN SPE GM TP TN",file = "parameter_optimize.txt",append = F,quote = F,row.names = F,col.names = F)
	for (i in -5:5){
		ii = 2^i
		for (j in -5:5){
			jj = 2^j
			...
			write.table(tem,'parameter_optimize.txt',append =T, quote =F, row.names =F,col.names =F)
		}
	}
}
